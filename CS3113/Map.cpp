...
Map::Map(int mapColumns, int mapRows, unsigned int* levelData, const char* textureFilePath, float tileSize,
...
void Map::build() {
    ...
    for (int row = 0; row < mTextureRows; row++) {
        for (int col = 0; col < mTextureColumns; col++) {
            Rectangle textureArea = {(float)col * (mTextureAtlas.width / mTextureColumns),
...
void Map::render() {
    ...
    for (int row = 0; row < mMapRows; row++) {
        ...
        for (int col = 0; col < mMapColumns; col++) {
            ...
            Rectangle destinationArea = {mLeftBoundary + col * mTileSize,
...
bool Map::isSolidTileAt(Vector2 position, float* xOverlap, float* yOverlap) {
    ...
    *yOverlap = 0.0f;
    ...
    if (position.x < mLeftBoundary || position.x > mRightBoundary || position.y < mTopBoundary ||
        position.y > mBottomBoundary)
        return false;

    int tileXIndex = floor((position.x - mLeftBoundary) / mTileSize);
    int tileYIndex = floor((position.y - mTopBoundary) / mTileSize);

    if (tileXIndex < 0 || tileXIndex >= mMapColumns || tileYIndex < 0 || tileYIndex >= mMapRows) return false;
    ...
    /*
    When our collision probe touches a solid tile, we calculate how far the probe has penetrated into that tile
    along each axis — this is what we call the overlap. Each tile has a center point and a known half-size (half of
    mTileSize). By comparing the probe’s position to the tile’s center, we find how deep the probe is inside the
    tile: if the distance between them is smaller than the tile’s half-size, then the probe is overlapping. The
    overlap value (mTileSize / 2) - fabs(position - tileCenter) tells us exactly how much to push the entity back
    so that it sits flush against the tile’s edge without clipping inside. We calculate this separately for both
    the X and Y axes, which lets us resolve collisions in either direction — for example, preventing the player
